...
def main(argv: list | None = None):
    ...
    parser.add_argument(
        "--llama-cpp-binary",
        required=False,
        help="Path or name of llama.cpp binary (optional)",
    )
    parser.add_argument(
        "--model-path",
        required=False,
        help="Path to local model file for adapters (ggml for llama.cpp or transformers local folder)",
    )
    parser.add_argument(
        "--use-transformers",
        action="store_true",
        help="Prefer local transformers adapter when a local model path is provided",
    )
    parser.add_argument(
        "--seed", required=False, type=int, default=42, help="Deterministic seed"
    )
    parser.add_argument(
        "--k", required=False, type=int, default=3, help="Number of docs to retrieve"
    )
    parser.add_argument(
    ...
    if args.use_transformers and args.model_path:
        try:
            model = LlamaAdapter(
                model_path=args.model_path, device="cpu", seed=args.seed
            )
            logger.info(
                "Using LlamaAdapter (transformers) with model=%s", args.model_path
            )
        ...
        try:
            model = LlamaCppAdapter(
                ...
                binary_path=args.llama_cpp_binary,
                seed=args.seed,
            )
            logger.info(
                "Using LlamaCppAdapter with binary=%s, model=%s",
                args.llama_cpp_binary,
                args.model_path,
            )
        except Exception as e:
            logger.exception(
                "Failed to initialize LlamaCppAdapter, falling back to stub: %s", e
            )
    ...
    docstore.add_documents(
        [
            {
                "id": "d1",
                "text": "Privacy policy: We do not store passwords.",
                "meta": {},
            },
            {
                "id": "d2",
                "text": "Terms: Users agree to share non-sensitive info.",
                "meta": {},
            },
        ]
    )
